...
var _ = Describe("KubeProxyInfo", func() {
	...
	Context("testing /kubeproxyinfo endpoint", func() {
		...
		It("should return the expected value of KubeProxyInfo", func() {
			jsonToCompare := &sensor.KubeProxyInfo{}
			jsonKubeProxyInfo := &sensor.KubeProxyInfo{}

			resBody, err = ioutil.ReadAll(res.Body)
			Expect(err).ToNot(HaveOccurred())

			err = json.Unmarshal(resBody, jsonKubeProxyInfo)
			Expect(err).ToNot(HaveOccurred())

			Expect(jsonKubeProxyInfo.KubeConfigFile).To(Equal(jsonToCompare.KubeConfigFile))
		})
	})
})
